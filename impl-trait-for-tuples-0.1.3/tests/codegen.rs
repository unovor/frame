...
#[test]
fn trait_with_associated_const() {
    trait TraitWithAssociatedConst {
        const ID: u32;
    }

    #[impl_for_tuples(1, 50)]
    impl TraitWithAssociatedConst for Tuple {
        for_tuples!( const ID: u32 = #( Tuple::ID )+*; );
    }

    struct Impl;

    impl TraitWithAssociatedConst for Impl {
        const ID: u32 = 1;
    }

    assert_eq!(1, Impl::ID);

    assert_eq!(3, <(Impl, Impl, Impl)>::ID);

    assert_eq!(5, <(Impl, Impl, Impl, Impl, Impl)>::ID);
}

#[test]
fn trait_with_associated_const_slice_type() {
    trait TraitWithAssociatedConst {
        const DATA: &'static [u32];
    }

    #[impl_for_tuples(1, 50)]
    impl TraitWithAssociatedConst for Tuple {
        for_tuples!( const DATA: &'static [u32] = &[ #( 0 ),* ]; );
    }
    ...
    assert_eq!(0, Impl::DATA.len());

    assert_eq!(3, <(Impl, Impl, Impl)>::DATA.len());

    assert_eq!(5, <(Impl, Impl, Impl, Impl, Impl)>::DATA.len());
}
